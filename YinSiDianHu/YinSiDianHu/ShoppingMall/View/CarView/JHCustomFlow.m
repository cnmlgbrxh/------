...
-(NSArray *)layoutAttributesForElementsInRect:(CGRect)rect{
    ...
    for(int i = 0; i < [attributes count]; i++) {
        UICollectionViewLayoutAttributes *attri = attributes[i];
        if(attri.representedElementKind == UICollectionElementKindSectionHeader || attri.representedElementKind == UICollectionElementKindSectionFooter) {
            continue;
        }
        if (i==0) {

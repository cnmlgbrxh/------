...
- (instancetype)initWithCNContact:(CNContact *)contact
{
    ...
    if (self)
    {
        ...
        {
            NSMutableString *mutableString = [NSMutableString stringWithString:self.fullName];
            CFStringTransform((CFMutableStringRef)mutableString, NULL, kCFStringTransformToLatin, false);
            self.pinyinName = [[mutableString stringByFoldingWithOptions:NSDiacriticInsensitiveSearch locale:[NSLocale currentLocale]] mutableCopy];
            NSMutableString *strm = [NSMutableString stringWithString:[self.pinyinName substringToIndex:1]];
            BOOL isOpen = NO;
            for(int i =0; i < [self.pinyinName length]; i++)
            {
                NSString *temp = [self.pinyinName substringWithRange:NSMakeRange(i, 1)];
                if (isOpen) {
                    isOpen = NO;
            ...
            self.firstZmName = strm;
            
            self.pinyinName = [self.pinyinName stringByReplacingOccurrencesOfString:@" " withString:@""];
        }
